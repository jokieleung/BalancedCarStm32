...
**************************************************************************/
int Amplitude=7200;   //Jokie修改为7000 //===PWM满幅是7200 限制在6900
...
//			if(Voltage_Count==100) Voltage=Voltage_All/100,Voltage_All=0,Voltage_Count=0;//=====求平均值		
			...
			Encoder_Left=-Read_Encoder(2);       //(我的平衡车的右电机编码器) 负值向前，正值向后                                  		
			Encoder_Right=Read_Encoder(4);      //(我的平衡车的左电机编码器) 负值向前，正值向后   读取编码器的值
	  	Get_Angle(Way_Angle);      			//===更新姿态	
...
// 	    Turn_Pwm    =turn(Encoder_Left,Encoder_Right,Gyro_Turn);            //===转向环PID控制     
 	  	...
 	  	Moto2=Balance_Pwm+Velocity_Pwm-Turn_Pwm;                            //===计算右轮电机最终PWM
			printf("限幅前PWM	MOTO1:	%d	\r\n",Moto1);		
			printf("限幅前PWM	MOTO2:	%d	\r\n",Moto2);		
			Xianfu_Pwm();                                                       //===PWM限幅
...
//      if(Turn_Off(Angle_Balance,Voltage)==0)                              //===如果不存在异常
			...
			printf("限幅后PWM	MOTO1:	%d	\r\n",Moto1);	
			printf("限幅后PWM	MOTO2:	%d	\r\n",Moto2);	
//PWMB=7200;PWMA=7200;
//			Set_Pwm(7200,7200);
//			printf("EXTI 中断IF内部程序完毕\r\n");			
	}       	
	 return 0;	  
} 
...
{  
   float Bias;
	 ...
	 balance=Balance_Kp*Bias+Gyro*Balance_Kd;   //===计算平衡控制的电机PWM  PD控制   kp是P系数 kd是D系数 KD*Gyro是与角速度成正比的阻尼力
	 return balance;
}
...
{  
   ...
   //=============速度PI控制器=======================//	
		...
		if(Flag_Stop==1)   Encoder_Integral=0;      //Turn_Off(Angle_Balance,Voltage)==1||===电机关闭后清除积分
	  return Velocity;
}
...
{
		static	int dead=4070;//死区
	   //4600左右2个轮子开始临界转动
		if(moto1<0)		{	AIN2=0,			AIN1=1;	moto1-=dead;if(moto1<=-Amplitude)moto1=-Amplitude;	}			//前进
			else 	      {   AIN2=1,			AIN1=0;	moto1+=dead;if(moto1>=Amplitude)moto1=Amplitude;	}		//后退
			PWMA=myabs(moto1);
		  if(moto2<0)	{BIN1=0,			BIN2=1;	moto2-=dead;if(moto2<=-Amplitude)moto2=-Amplitude;	}				//前进
			else       { BIN1=1,			BIN2=0;	moto2+=dead;if(moto2>=Amplitude)moto2=Amplitude;		}			//后退
			PWMB=myabs(moto2);	
}
